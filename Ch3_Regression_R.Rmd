...
lmfit_2008<- lm(log(`SalePrice`) ~ `Year Remod/Add`+`Year Built`+`Lot Area`+`Garage Area` + `Gr Liv Area`+`Year Remod/Add`*`Year Built`, data = ames_raw_2008)
```
...
~~~
It's better than the null model. But the loss is still large.
...
~~~
No. It may make the model to complex and cause some overfitting problems
...
~~~
The interaction in my model does not make sense according to the p-value. My model does not indicate signs of interaction
...
response_variable <- log(ames_raw_2008$SalePrice)

# List of predictors
predictors <- c("Year Remod/Add", "Year Built", "Lot Area", "Garage Area", "Gr Liv Area")
model1 <- lm(log(`SalePrice`) ~ `Year Remod/Add`^3+`Year Built`+`Lot Area`+`Garage Area` + `Gr Liv Area`+`Year Remod/Add`*`Year Built`, data = ames_raw_2008)
summary(model1)
pred1 <- exp(predict(model1,newdata=ames_raw_2009))
loss1 <- calc_loss(pred1,ames_raw_2009$SalePrice)

model2 <- lm(log(`SalePrice`) ~ `Year Remod/Add`+`Year Built`^3+`Lot Area`+`Garage Area` + `Gr Liv Area`+`Year Remod/Add`*`Year Built`, data = ames_raw_2008)
summary(model2)
pred2 <- exp(predict(model2,newdata=ames_raw_2009))
loss2 <- calc_loss(pred2,ames_raw_2009$SalePrice)

model3 <- lm(log(`SalePrice`) ~ `Year Remod/Add`+`Year Built`+`Lot Area`^3 +`Garage Area` + `Gr Liv Area`+`Year Remod/Add`*`Year Built`, data = ames_raw_2008)
summary(model3)
pred3 <- exp(predict(model3,newdata=ames_raw_2009))
loss3 <- calc_loss(pred3,ames_raw_2009$SalePrice)

model4 <- lm(log(`SalePrice`) ~ `Year Remod/Add`+`Year Built`+`Lot Area` +`Garage Area`^3 + `Gr Liv Area`+`Year Remod/Add`*`Year Built`, data = ames_raw_2008)
summary(model4)
pred4 <- exp(predict(model4,newdata=ames_raw_2009))
loss4 <- calc_loss(pred4,ames_raw_2009$SalePrice)

model5 <- lm(log(`SalePrice`) ~ `Year Remod/Add`+`Year Built`+`Lot Area` +`Garage Area` + `Gr Liv Area`^3+`Year Remod/Add`*`Year Built`, data = ames_raw_2008)
summary(model5)
pred5 <- exp(predict(model5,newdata=ames_raw_2009))
loss5 <- calc_loss(pred5,ames_raw_2009$SalePrice)
```

Your answer:

There does not show non-linear association in my model since the loss would not chance after cubed the predictor.
...
Your code:
```{r,echo=TRUE}
price_dif <- log(ames_raw_2009$SalePrice) - pred_2009
top_indices <- which(price_dif %in% head(sort(price_dif, decreasing = TRUE), 5))
selected_rows <- ames_raw_2009[top_indices, ]
selected_rows
```
...
~~~
The top 5 overpriced houses are shown above.
...
Your code:
```{r,echo=TRUE}
top_indices1 <- which(price_dif %in% head(sort(price_dif, decreasing = FALSE), 5))
selected_rows1 <- ames_raw_2009[top_indices1, ]
selected_rows1
```
...
~~~
The top 5 most good deal houses are shown above.
...
~~~
There is a relationship between predictor and the response.The Multiple R-squared value is 0.6059, indicating that approximately 60.59% of the variability in mpg can be explained by the predictor horsepower.The relationship between horsepower and mpg is negative. The predicted value of mpg with horsepower 98 is 24.46708, the 95% C.I. is [23.97308,24.96108].
...
Your code:
```{r,echo=TRUE}
plot(auto$horsepower, auto$mpg, main = "Scatterplot of mpg against horsepower", 
     xlab = "Horsepower", ylab = "MPG")

# Adding the least squares regression line
abline(fit_auto, col = "red")
```
...
ggplot(auto, aes(x = horsepower, y = mpg)) +
  ...
  labs(title = "Scatterplot of mpg against horsepower",
       x = "Horsepower",
       y = "MPG")
```
...
~~~
The plot matrix is shown above.
...
~~~
Yes, there is evidence of a relationship between the predictors and the response variable. Year, origin,weight is highly statistically significant.
...
~~~
There are some outliers when fitted values are large. The leverage plot identify an observations with unusually high leverage.
...
Your code:
```{r,echo=TRUE}
model_with_interaction <- lm(mpg ~ . - name + weight:year, data = auto)
...
~~~
The weight:year interactions appear to be statistically significant. 
...
```{r,echo=TRUE}
model_log <- lm(mpg ~ log(weight) + log(horsepower) + log(displacement), data = auto)
model_sqrt <- lm(mpg ~ sqrt(weight) + sqrt(horsepower) + sqrt(displacement), data = auto)
model_sq <- lm(mpg ~ weight^2 + horsepower^2 + displacement^2, data = auto)
...
fit_carseat <- lm(Sales ~ Price + Urban + US, data = carseat)
summary(fit_carseat)
```
...
~~~
Urban shows not significant in this model.
...
~~~
The intercept represents the estimated Sales when all predictor variables are zero. For every one-unit increase in the Price of the product, Sales are expected to decrease by approximately 0.054 units, holding other variables constant.The coefficient for 'UrbanYes' represents the change in Sales associated with the Urban variable being "Yes" compared to being "No", while holding other variables constant.The coefficient for 'USYes' represents the change in Sales associated with the US variable being "Yes" compared to being "No", while holding other variables constant. 
...
~~~
Sales = 13.04 − 0.054 × Price − 0.022 × UrbanYes + 1.201 × USYes + ϵ
...
~~~
We reject the null hypothesis for the intercept, Price, and USYes, indicating that these variables are statistically significant in predicting Sales.
...
Your code:
```{r,echo=TRUE}
smaller_model <- lm(Sales ~ Price + US, data = carseat)
...
~~~
Comparing the adjusted R-squared values, the smaller model has a slightly higher value (0.2354) compared to the previous model (0.2335), indicating a slightly better fit. The residual standard errors are very close, and the F-statistic for the smaller model is higher, suggesting that the smaller model explains the variability in Sales better.
...
~~~
The C.I. for the coefficients are shown above.
...
~~~
It's approximately around 0 so there are no evidence of outliers.
